...
def sr_dqn_train(rand_seed = 0, soft_max = False): 
    gif_plotting = False      
    monkey_name = "p" 
    bool_pre_train = True 
    bool_PER = False 
    ...
    data_dir, data_saver, writer = directory_setting(game_name, monkey_name,
        task_info=f"simple {game_name} training with pretrain {bool_pre_train} and PER {bool_PER}", run_num=rand_seed)
    ...
    agent = agent_package.SuccessorRepresentAgent(state_size = state_size, 
