...
def simple_dqn_train(rand_seed = 0, soft_max = False, fixed_trial = None): 
    gif_plotting = False      
    monkey_name = "p" 
    bool_pre_train = True 
    bool_PER = False 
    ...
    game_name = "Valina_dqn" if not(soft_max) else "Softmax_dqn"
    ...
    if not(soft_max):
        ...
        agent = agent_package.SoftmaxDqnAgent(state_size = state_size,
                                              action_size = action_size,
                                              hidden_size = hidden_size,
                                              learning_rate = learning_rate,
                                              memory_size = memory_size,
    ...
    for trial_num in range(trial_length):
        if not(fixed_trial is None):
            trial_num = trial_changed_start[fixed_trial]
        ...
        while not(done or truncated):
            ...
            plotting_functions.plot_all_functions(agent=agent, model=None, i_episode = trial_num, trial_t = trial_t, 
                                                state = state, reward_location=trial_goal, data_dir=data_dir)
